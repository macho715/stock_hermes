...
This append-only architecture note records the active `root_folder_snapshot/stock-pred-v5` safety gate path.

Runtime component contract:
- `StockPredV5.jsx` owns snapshot validation and `buildExecutiveSafetyState`.
- `RecommendationKpi.jsx`, `ConfidenceKpi.jsx`, and `RiskRewardKpi.jsx` consume `safetyState` for the top KPI row.
- `AiDecisionPanel.jsx` shows the blocker banner and passes `safetyState` to `ActionPlanPanel.jsx`.
- `ActionPlanPanel.jsx` disables entry, stop loss, TP1, and TP2 when `actionPlanEnabled` is false.
- `WatchlistPanel.jsx` shows the selected blocked ticker as `NO TRADE`.

Verification evidence:
- `npm run build` passed in `root_folder_snapshot/stock-pred-v5`.
- Browser verification on `http://127.0.0.1:5173/` confirmed AAPL and MSFT show `NO TRADE / PAPER ONLY`.

```mermaid
flowchart TB
  RecAPI[/api/recommend] --> StockPred[StockPredV5.jsx]
  StockPred --> Gate[buildExecutiveSafetyState]
  Gate --> TopKpi[Top KPI components]
  Gate --> AiPanel[AiDecisionPanel.jsx]
  AiPanel --> ActionPanel[ActionPlanPanel.jsx disabled]
  Gate --> Watchlist[WatchlistPanel.jsx selected row]
```
