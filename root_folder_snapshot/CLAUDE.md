...
- LLM Advisor (`advisor_run=True`) requires `ANTHROPIC_API_KEY` or `MINIMAX_API_KEY`; `api_server.py` silently disables it when both keys are absent — never assume the advisor ran unless the response contains `advisor_score`. Set `LLM_ADVISOR_PROVIDER=minimax` to force MiniMax when both providers are configured. Dashboard toggle (`advisorEnabled`) is `false` by default and appends `advisor_run=1&advisor_blend_weight=0.3` to `/api/recommend` only when explicitly enabled.
