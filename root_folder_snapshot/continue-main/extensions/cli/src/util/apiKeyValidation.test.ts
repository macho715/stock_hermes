...
describe("isValidAnthropicApiKey", () => {
  it("should return true for valid API keys", () => {
    expect(isValidAnthropicApiKey("ANTHROPIC_KEY_EXAMPLE_SHORT")).toBe(true);
    expect(isValidAnthropicApiKey("ANTHROPIC_KEY_EXAMPLE_ALPHA")).toBe(true);
    expect(isValidAnthropicApiKey("ANTHROPIC_KEY_EXAMPLE_DASHED")).toBe(true);
    expect(
      isValidAnthropicApiKey("ANTHROPIC_KEY_EXAMPLE_LONG"),
    ).toBe(true);
  });
