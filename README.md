...
Microsoft RD-Agent (NeurIPS 2025) automates factor discovery via Docker subprocess.
...
```bash
# 1단계: 팩터 마이닝 실행
python main.py factor-mine --cycles 2 --budget 10.0

# 2단계: 발견된 팩터 확인
python main.py factor-list

# 3단계: 팩터 검토 후 승인
python main.py factor-approve --session rd_20260529_abc123 --names rd_momentum,rd_reversal
