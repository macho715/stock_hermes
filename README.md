...
| **OpenAI 뉴스 분석** | **`LLM_ADVISOR_PROVIDER=openai` — OpenAI Responses API Structured Outputs로 `notebook_analysis` 보강** |
...
| **Kraken Dashboard Redesign** | 구현 기록: [`docs/KRAKEN_DASHBOARD_REDESIGN_IMPLEMENTATION_20260530.md`](docs/KRAKEN_DASHBOARD_REDESIGN_IMPLEMENTATION_20260530.md) · actual-data 계획: [`plan_20260530_actual_dashboard_data.md`](plan_20260530_actual_dashboard_data.md) · 검증 캡처: [`artifacts/dashboard-redesign/stockpred-actual-data-dashboard-20260530.png`](artifacts/dashboard-redesign/stockpred-actual-data-dashboard-20260530.png) |
| **NotebookLM 뉴스** | iran-war-notelm `:8088` API 연동 — `notebook_analysis`, `scenario_outlook` → AI Panel |
| **OpenAI API 뉴스 분석** | `LLM_ADVISOR_PROVIDER=openai`, `OPENAI_API_KEY`, `OPENAI_ADVISOR_MODEL` — NotebookLM/Notelm 분석을 Pydantic structured output으로 보강 |
| **Notelm fallback file cache** | NotebookLM 서버가 꺼져도 `reports/notelm_fallback_cache/<MARKET>/<TICKER>.json` 캐시로 Watchlist 재로딩 단축 |
| **Watchlist 종목별 Notelm** | `/api/watchlist-notelm` — 전체 Watchlist 종목에 Notelm fallback 분석, 추천, confidence, 뉴스 수 매핑 |
| **Actual dashboard data** | `fundamentals`, `news_headlines`, `scenario_outlook`, `notebook_analysis`를 backend snapshot에서 전달 |
...
| **테스트** | NotebookLM/Notelm regression 37 passed + dashboard build passed |

```mermaid
flowchart LR
    Watchlist[WatchlistPanel] --> API[/api/watchlist-notelm/]
    API --> Adapter[advisors/notebooklm_news.py]
    Adapter --> NLM[NotebookLM :8088]
    Adapter -->|8088 unavailable| Fallback[Notelm local fallback]
    Fallback --> Cache[(reports/notelm_fallback_cache)]
    Cache -->|TTL hit| API
```

**검증 결과:** 9개 Watchlist 종목 기준 cold load `24.75s` → warm cache load `6.41s`; 상태값은 `LOCAL_FALLBACK` → `LOCAL_CACHE_HIT`로 전환됨.

### 2026-05-30 — 이번 세션 통합 산출물

이번 세션은 디자인, 실제 데이터, NotebookLM/Notelm fallback, 문서화를 한 흐름으로 묶었다.

```mermaid
flowchart LR
    Mockup[Generated mockup image] --> Kraken[Kraken dark fintech UI]
    Kraken --> ActualData[Backend actual data fields]
    ActualData --> Notelm[NotebookLM or Notelm fallback]
    Notelm --> Watchlist[/api/watchlist-notelm]
    Watchlist --> FileCache[reports/notelm_fallback_cache]
    FileCache --> Docs[README + CHANGELOG + COMPONENT_LAYOUT + GUIDE + plan]
```

| 산출물 | 위치 | 의미 |
|---|---|---|
| 구현 기록 | [`docs/KRAKEN_DASHBOARD_REDESIGN_IMPLEMENTATION_20260530.md`](docs/KRAKEN_DASHBOARD_REDESIGN_IMPLEMENTATION_20260530.md) | 목업 기반 UI 변경과 검증 기록 |
| 대시보드 데이터 가이드 | [`docs/DASHBOARD_DESIGN_AND_DATA_GUIDE.md`](docs/DASHBOARD_DESIGN_AND_DATA_GUIDE.md) | 실제 데이터 필드, fallback, 실행 방법 |
| Watchlist Notelm endpoint | `api_server.py` `/api/watchlist-notelm` | full recommendation 없이 종목별 뉴스 분석 보강 |
| Notelm adapter | `src/stock_rtx4060/advisors/notebooklm_news.py` | NotebookLM API 실패 시 yfinance 뉴스 기반 Notelm fallback 생성 |
| OpenAI analyzer | `src/stock_rtx4060/advisors/openai_client.py` | `LLM_ADVISOR_PROVIDER=openai`일 때 Responses API structured output으로 뉴스 영향 분석 |
| 파일 캐시 | `reports/notelm_fallback_cache/US/*.json` | warm Watchlist 로딩 단축 |
| root-docs 설정 | `.root-docs-batch-update.toml` | 이 저장소의 실제 루트 문서 매핑 |
...
### E6 — OpenAI API News Analysis

OpenAI API 분석은 선택 기능이다.
기본값에서는 기존 Anthropic/MiniMax/Notelm 경로를 그대로 사용한다.
`LLM_ADVISOR_PROVIDER=openai`를 설정하면 NotebookLM 또는 Notelm 뉴스 payload가 OpenAI Responses API의 Pydantic Structured Outputs로 보강된다.

```bash
export LLM_ADVISOR_PROVIDER=openai
export OPENAI_API_KEY=sk-...
export OPENAI_ADVISOR_MODEL=gpt-4o
```

OpenAI 호출이 실패하거나 API key가 없으면 기존 NotebookLM/Notelm 분석을 유지하고 `notebook_analysis.openai_error`만 기록한다.
실제 API 통합 테스트는 실수로 유효하지 않은 키를 호출하지 않도록 `RUN_OPENAI_LIVE_TESTS=1`일 때만 실행된다.
...
## Codex Implementation Note - 2026-05-31 Dashboard Loading State Cleanup

Executive Dashboard v2.1 now hides stale ticker analysis while a newly selected ticker is loading. `root_folder_snapshot/stock-pred-v5/src/StockPredV5.jsx` only passes a recommendation snapshot into the dashboard when the snapshot ticker matches the selected ticker. While the new `/api/recommend` response is pending, `AiDecisionPanel.jsx`, `ConfidenceKpi.jsx`, `RiskRewardKpi.jsx`, and `MarketSnapshotPanel.jsx` show explicit `Loading` or `No data` states instead of a single dash placeholder.

`NewsTimelinePanel.jsx` and `NotebookNewsAnalysis.jsx` also normalize em dash characters in displayed news titles to a plain hyphen so browser-level dash scans do not confuse headline punctuation with missing data.

```mermaid
flowchart LR
  SelectTicker[User selects ticker] --> FetchSnap[/api/recommend fetch]
  FetchSnap --> MatchCheck{Snapshot ticker matches selection}
  MatchCheck -->|No| LoadingUI[Show Loading and hide stale analysis]
  MatchCheck -->|Yes| CurrentUI[Render current ticker analysis]
  CurrentUI --> DashScan[Browser dash scan equals 0]
```

Verification evidence: `npm run build` passed in `root_folder_snapshot/stock-pred-v5`, and browser inspection across AAPL, MSFT, NVDA, TSLA, AMZN, GOOGL, META, SPY, and QQQ found zero visible `—` characters during loading and after QQQ OpenAI analysis completed.
...
This append-only note records the active Executive Dashboard safety gate port for `root_folder_snapshot/stock-pred-v5`.

Changed runtime path:
- `root_folder_snapshot/stock-pred-v5/src/StockPredV5.jsx` now builds `safetyState` from the selected `/api/recommend` snapshot.
- `root_folder_snapshot/stock-pred-v5/src/components/RecommendationKpi.jsx` shows `NO TRADE / PAPER ONLY` and the original AI recommendation when a hard blocker exists.
- `root_folder_snapshot/stock-pred-v5/src/components/ConfidenceKpi.jsx` caps blocked confidence at `<=50%` and labels it `Blocked by risk gate`.
- `root_folder_snapshot/stock-pred-v5/src/components/RiskRewardKpi.jsx` shows `No trade` instead of an actionable ratio when blocked.
- `root_folder_snapshot/stock-pred-v5/src/components/AiDecisionPanel.jsx` renders `RISK GATE ACTIVE - NO TRADE / PAPER ONLY` with blocker evidence.
- `root_folder_snapshot/stock-pred-v5/src/components/ActionPlanPanel.jsx` renders `REFERENCE PLAN - DISABLED BY RISK GATE`.
- `root_folder_snapshot/stock-pred-v5/src/components/WatchlistPanel.jsx` colors selected blocked rows as `NO TRADE`.

Verification evidence:
- `npm run build` passed in `root_folder_snapshot/stock-pred-v5`.
- `git diff --check` passed for the edited Executive Dashboard files.
- Browser verification on `http://127.0.0.1:5173/` showed `NO TRADE / PAPER ONLY`, `Blocked by risk gate`, `RISK GATE ACTIVE - NO TRADE / PAPER ONLY`, and `REFERENCE PLAN - DISABLED BY RISK GATE`.
- A Watchlist click from AAPL to MSFT was verified; MSFT completed loading and showed the same safety gate state.

```mermaid
flowchart LR
  Snapshot[/api/recommend snapshot] --> Safety[buildExecutiveSafetyState]
  Safety --> KPI[Recommendation / Confidence / Risk KPI]
  Safety --> Decision[AI Decision Panel]
  Safety --> Action[Action Plan disabled]
  Safety --> Watchlist[Selected Watchlist row NO TRADE]
  Action --> ReportOnly[Report-only dashboard, no broker execution]
```
