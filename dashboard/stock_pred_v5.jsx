...
export default function StockPredV5() {
  ...
  const backendResults = backendSnapshot?.results || [];
  const backendCurrent = useMemo(
    ...
    reader.onload = () => {
      try {
        ...
        if (payload.schema_version !== "dashboard_snapshot.v1" || !Array.isArray(payload.results)) {
          throw new Error("dashboard_snapshot.v1 JSON required");
        }
        ...
        setBackendError(err instanceof Error ? err.message : "snapshot import failed");
        setTab("BACKEND");
      } finally {
        event.target.value = "";
      }
        ...
        <div className="flex items-center justify-between gap-4 flex-wrap">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              style={{
                padding: "6px 12px", background: backendSnapshot ? C.panelHi : "transparent",
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              {["SIGNAL", "MODELS", "BACKTEST", "BACKEND"].map((t) => (
                <button
                  key={t}
                  onClick={() => setTab(t)}
                  style={{
...
/* ----------  BACKEND REPORT TAB  ---------- */
function BackendTab({ snapshot, currentResult, backendError, accent }) {
  const fmtNum = (v, digits = 2) => (typeof v === "number" ? v.toFixed(digits) : "—");
  const rows = snapshot?.results || [];
  const selected = currentResult || rows[0] || null;

  if (backendError) {
    return (
      <Panel title="BACKEND SNAPSHOT" right="ERROR" rightColor={C.red}>
  ...
  if (!snapshot) {
    return (
      <Panel title="BACKEND SNAPSHOT" right="EMPTY" rightColor={C.textDim}>
        <div style={{ padding: 12, color: C.textDim, fontSize: 11, lineHeight: 1.6 }}>
          NO BACKEND SNAPSHOT
        </div>
      </Panel>
    );
  ...
  return (
    <>
      <Panel title="BACKEND REPORT SNAPSHOT" right="REPORT ONLY" rightColor={C.green}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6, padding: 4 }}>
          <BackendKV label="MODE" value={snapshot.mode || "report_only"} color={C.green} />
          <BackendKV label="RESULTS" value={String(snapshot.result_count ?? rows.length)} color={accent} />
          <BackendKV label="SOURCE" value={snapshot.source || "stock_rtx4060_unified"} />
          <BackendKV label="FILE" value={snapshot.importedFileName || "dashboard_snapshot.json"} />
        </div>
        <div style={{ padding: "4px 6px", color: C.textMuted, fontSize: 9, lineHeight: 1.5 }}>
          {snapshot.audit_log_path || "audit_log_path unavailable"}
        </div>
      </Panel>
      ...
      {selected && (
        <Panel title={`BACKEND · ${selected.ticker} · TRACK-${selected.track}`} right={selected.verdict} rightColor={verdictColor(selected.verdict)}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 6, padding: 4 }}>
            ...
            <BackendKV label="ENTRY" value={fmtNum(selected.entry)} />
            <BackendKV label="STOP" value={fmtNum(selected.stop)} color={C.red} />
          ...
          <div style={{ marginTop: 8 }}>
            {(selected.validations || []).slice(0, 8).map((v) => (
              <div
                key={`${v.name}-${v.status}`}
                style={{
                  padding: "5px 6px", marginBottom: 4, background: C.bgDeep,
                  borderLeft: `2px solid ${v.status === "PASS" ? C.green : v.status === "AMBER" ? C.amber : C.red}`,
                }}
              >
                <div className="flex justify-between">
                  <span style={{ color: C.textDim, fontSize: 9, letterSpacing: 1 }}>{v.name}</span>
                  <span style={{ color: v.status === "PASS" ? C.green : v.status === "AMBER" ? C.amber : C.red, fontSize: 9 }}>{v.status}</span>
                </div>
                <div style={{ color: C.text, fontSize: 9, lineHeight: 1.4, marginTop: 2 }}>{v.evidence}</div>
              </div>
            ))}
          </div>
          <AdvisorOverlay
      ...
      <SectionLabel>BACKEND RANKING</SectionLabel>
      <div style={{ maxHeight: 250, overflowY: "auto" }}>
        {rows.slice(0, 10).map((r) => (
          <div
            key={`${r.rank}-${r.ticker}-${r.track}`}
            style={{
              display: "grid", gridTemplateColumns: "28px 64px 1fr 52px",
              alignItems: "center", padding: "5px 6px", marginBottom: 3,
              background: C.bgDeep, borderLeft: `2px solid ${verdictColor(r.verdict)}`,
              fontSize: 10,
            }}
          >
...
function BackendKV({ label, value, color }) {
  return (
