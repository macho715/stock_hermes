...
This package keeps the historical ``stock_rtx4060.reports`` API while also
hosting submodules such as ``stock_rtx4060.reports.model_card``.
"""
...
from .model_card import build_model_card, write_model_card
...
class ReportWriter:
    ...
    def risk_dashboard(self, candidates: Iterable[CandidateVerdict], config: RiskConfig | None = None, filename: str | None = None) -> Path:
        ...
        lines = [
            ...
            "## Capital Buckets",
            "",
            portfolio_targets(cfg).to_markdown(index=False),
            "",
            "## Open Risk",
            "",
            f"- Open risk: {open_risk:,.2f}",
            f"- Track-S open-risk limit: {cfg.track_s_capital * cfg.max_open_risk_pct:,.2f}",
            f"- Status: {'PASS' if open_risk <= cfg.track_s_capital * cfg.max_open_risk_pct else 'FAIL'}",
            "",
            "## Gate Counts",
            "",
        ]
        ...
        for candidate in l_candidates:
            lines.extend(
                [
                    f"## {candidate.ticker}",
                    "",
                    f"- Gate: {candidate.gate.value}",
                    f"- Score: {candidate.score:.2f}",
                    f"- Verdict: {candidate.verdict}",
                    f"- Max single-name position value under policy: {candidate.position_value:,.2f}",
