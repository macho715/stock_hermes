...
Changed:
- Ported the `NO TRADE / PAPER ONLY` safety gate into `root_folder_snapshot/stock-pred-v5/src/StockPredV5.jsx`.
- Connected `safetyState` into Recommendation, Confidence, Risk/Reward, AI Decision, Action Plan, and Watchlist components.
- Disabled actionable Entry, Stop Loss, TP1, and TP2 display when hard blockers are present.

Verified:
- `npm run build` passed in `root_folder_snapshot/stock-pred-v5`.
- `git diff --check` passed for the edited Executive Dashboard files.
- Browser verification on `http://127.0.0.1:5173/` showed the safety gate for AAPL and MSFT.

```mermaid
timeline
  title Executive Safety Gate Port
  Patch : StockPredV5 safetyState
  Connect : KPI and decision components
  Verify : Build and browser AAPL/MSFT
```
